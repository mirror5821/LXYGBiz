...
import dev.mirror.library.utils.IdNumUtil;
...
public class ZiZhi2Fragment extends BaseFragment {
	 ...
	 * 当前所有的省列表
	 */
	private final List<CityFromDB> mCurrentProvices = new ArrayList<CityFromDB>();
	private final ArrayList<String> mPros = new ArrayList<String>();
	private String mCurrentProName;
	/*
	 * 当前省的市列表
	 */
	private final List<CityFromDB> mCurrentCity = new ArrayList<CityFromDB>();
	private final ArrayList<String> mCitys = new ArrayList<String>();
	...
	public boolean personIdValidation(String text) {
		...
		switch (text.length()) {
		case 18:
			isTure =IdNumUtil.validateIdCard18(text);
			break;
		case 15:
			isTure= IdNumUtil.validateIdCard15(text);
			break;
		}
		return isTure;
	...
	private void sub(){
		...
		mHttpClient.postData1(SHOP_ZIZHI, new AppAjaxParam(jb),
			...
			public void onResult(String data, String msg) {
				...
				mUser = SharePreferencesUtil.getShopInfo(getActivity());
				mUser.setQ_verifi(1);
				SharePreferencesUtil.saveShopInfo(getActivity(), JsonUtils.beanToString(mUser, User.class));
			}
